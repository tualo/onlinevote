Ext.define('Tualo.OnlineVote.controller.Ballotbox', {
  ...
  generateKey: function (name) {
    (async () => {
      ...
      try {
        let fresponse = await fetch('./onlinevote/append/publickey', {
          method: 'POST',
          headers: {
        ...
        if (fresponse.status == 403) {
          Ext.toast({
            html: 'Sie haben keine Berechtigung zum Anlegen einer Urne.',
            title: 'Fehler',
            align: 't',
            iconCls: 'fa fa-warning'
          });
        ...
        let response = await fresponse.json();
        if (response.success) {
        ...
        Ext.toast({
          html: 'Fehler beim Anlegen der Urne: ' + e.message,
          title: 'Fehler',
          align: 't',
          iconCls: 'fa fa-warning'
        });
